...
namespace Demo_Neporojnev
{
    public partial class Form1 : Form
    {
        ...
        {
            List<product> list = conn.getSelectedProduct(sqlCommand);
            if (list.Count % 20 != 0)
            {
        ...
        public void productToFLP()
        {
            ...
            for (int i = 0 + (20 * (currentPage - 1)); i < iEnd + (20 * (currentPage - 1)); i++)
            {
                ...
                {
                    uct.pictureOfProduct.ImageLocation = $"C:\\Users\\Admin\\Desktop\\Промежуточный контроль\\Сессия 1{uct.product.image}";
                }
                else
                {
                    uct.pictureOfProduct.ImageLocation = $"C:\\Users\\Admin\\Desktop\\Промежуточный контроль\\Сессия 1\\picture.png";
                }
